...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.Tensor(self.n_features * self.embed_size, self.hidden_size)) #W
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.Tensor(self.hidden_size, )) #b1
        nn.init.uniform_(self.embed_to_hidden_bias)
        self.dropout = nn.Dropout(p=self.dropout_prob) #dropout layer
        self.hidden_to_logits_weight = nn.Parameter(torch.Tensor(self.hidden_size, self.n_classes)) #U
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(torch.Tensor(self.n_classes, )) #b2
    ...
    def embedding_lookup(self, w):
        ...
        x = x.view(w.size()[0], self.n_features * self.embed_size)
        return x
    ...
    def forward(self, w):
        ...
        """
        hidden_layer = torch.nn.functional.relu(self.embedding_lookup(w) @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        logits = self.dropout(hidden_layer) @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
        return logits

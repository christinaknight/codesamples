...
def naiveSoftmaxLossAndGradient(
    ...
    probs = softmax(np.dot(centerWordVec, outsideVectors.T))
    loss = -1 * np.log(probs)[outsideWordIdx]
    y = np.zeros(len(probs))
    y[outsideWordIdx] = 1
    gradCenterVec = np.matmul(outsideVectors.T, probs) - np.matmul(outsideVectors.T, y)
    probs[outsideWordIdx] -= 1
    gradOutsideVecs = np.dot(probs[:, np.newaxis], centerWordVec[np.newaxis, :])
...
def negSamplingLossAndGradient(
    ...
    uovc = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    ukvc = np.dot(outsideVectors[negSampleWordIndices], centerWordVec)
    loss = -1 * np.log(sigmoid(uovc)) - np.sum(np.log(sigmoid(-1 * ukvc)))
    c_firstarg = (sigmoid(uovc) - 1) * outsideVectors[outsideWordIdx]
    c_secondarg = np.sum((sigmoid(- ukvc) - 1)[:, np.newaxis] * outsideVectors[negSampleWordIndices], axis = 0)
    gradCenterVec = c_firstarg - c_secondarg
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(uovc) - 1) * centerWordVec
    for i in range(len(negSampleWordIndices)):
        gradOutsideVecs[negSampleWordIndices[i]] += (-1 * (sigmoid(-1 * ukvc) - 1)[i]) * centerWordVec
        # print(gradCenterVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for u_o in outsideWords:
        o_ind = word2Ind[u_o]
        o_loss, o_grad_center, o_grad_others = word2vecLossAndGradient(v_c, o_ind, outsideVectors, dataset)
        gradOutsideVectors += o_grad_others
        gradCenterVecs[word2Ind[currentCenterWord]] += o_grad_center
        loss += o_loss
